...
def get_driver_earnings(driver_user_id: int, db: Session = Depends(get_db)):
    earnings = (
...
def create_driver_earning(driver_user_id: int, payload: DriverEarningRequest, db: Session = Depends(get_db)):
    ...
    existing_earning = db.query(DriverEarning).filter(DriverEarning.ride_id == payload.ride_id).first()
    ...
    app_fee_value = payload.gross_value * APP_FEE_PERCENTAGE
    net_value = payload.gross_value - app_fee_value
