...
INITIAL_RIDE_STATUS = "AGUARDANDO_ACEITE"
...
def quote_ride(payload: RideQuoteRequest):
...
def create_ride(payload: RideCreateRequest, db: Session = Depends(get_db)):
    ...
    initial_status = (
        db.query(RideStatus)
        .filter(RideStatus.status == INITIAL_RIDE_STATUS)
        .first()
    )
    if not initial_status:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ride status {INITIAL_RIDE_STATUS} was not found.",
        )

    quote = quote_ride(payload)
    ride = Ride(
